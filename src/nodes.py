...
class Node(ABC):
    @abstractmethod
    def at_time(self, t: int) -> Optional[list[Message]]:
        '''Method to be called at specific time step.'''
    
    @abstractmethod
    def on_message_received(self, msg: Message) -> Optional[list[Message]]:
...
class HonestNode(Node):
    ...
    async def at_time(self, t: int) -> Optional[list[Message]]:
        ...
        if leader == self.idx and (t / (2*self.delta)).is_integer():
            ...
            messages = [Message(received_from=self.idx, send_to=idx, content=self.block) for idx in self.idxs]
            return messages
    ...
    def _(self, content: Block) -> list[Message]:
        self.block = content
        self.votes_for_block = 0
        self.nodes_voted = []
        head = self._head_longest_notarized()
        if self.block.parent == head:
            ...
            messages = [Message(received_from=self.idx, send_to=idx, content=vote) for idx in self.idxs]
            return messages
    ...
    def _notarize_block(self, block_to_notarize: Block) -> None:
        ...
        for block_notarized in self.notarized:
            if block_notarized.block_id == block_to_notarize.parent and len(self.notarized.nodes) > 1:
    ...
    def _chains(self) -> list[list[Block]]:
        ...
        chains = [chain for leaf in leaves for chain in nx.all_simple_paths(self.notarized, root, leaf)]
        return chains

    def _finalize_blocks(self, chains) -> None:
        for chain in chains:
            chain_reversed = list(reversed(list(enumerate(chain))))
            chain_reversed_chunked = [chain_reversed[i:i + 3] for i in range(0, len(chain_reversed), 3)]
            for chunk in chain_reversed_chunked:
                if len(chunk) == 3 and chunk[0][1].epoch == chunk[1][1].epoch + 1 and chunk[1][1].epoch == chunk[2][1].epoch + 1:
                    idx_final = chunk[2][0]
    ...
    def _(self, content: Vote) -> None:
        ...
        if content.block_id == self.block.block_id:
            self.votes_for_block += 1
            if self.votes_for_block == 2*self.max_dishonest + 1:
                block_to_notarize = copy.deepcopy(self.block)  # current block can be mutated in the future
